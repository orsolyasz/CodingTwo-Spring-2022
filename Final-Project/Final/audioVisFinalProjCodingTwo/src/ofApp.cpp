...
void ofApp::update(){
    ...
    for(int y=0; y<h; y++) {
        for(int x=0; x<w; x++) {
            ...
            float mappedi = ofMap(y,0,60,vidGrabber.getHeight(),0) * vidGrabber.getWidth() + ofMap(x,0,80, vidGrabber.getWidth(),0);
...
//    if(camvis){
//    grayImage.draw(vidGrabber.getWidth()+20,20, -ofGetWidth(),ofGetHeight());
//    }
...
//        ofSetColor(drawColor);
        ...
        drawColor.setHsb(120+120/bandsToGet* i, 255 , 255, 50);
        ofSetColor(drawColor);
        //ofDrawCircle(20+4*i, 5+i, val[i]*length*0.02);
        ofDrawCircle(0+i*1.15,400+200*sin(i*0.11+time*1.2)+10, val[i]*length*0.005);
